package me.ender;

import haven.*;

import static haven.MCache.*;

public class CustomCursors {
    public static final Resource.Named inspectCursor = Resource.local().loadwait("gfx/hud/curs/studyx").indir();
    public static final Resource.Named trackCursor = Resource.local().loadwait("gfx/hud/curs/track").indir();


    public static boolean processHit(MapView map, Coord2d mc, ClickData inf) {
	UI ui = map.ui;

	if(inf != null) {
	    Gob gob = Gob.from(inf.ci);
	    if(gob != null) {
		if(isTracking(map)) {
		    ui.gui.mapfile.track(gob);
		    stopTracking(map);
		    return true;
		}
	    }
	}

	return false;
    }

    public static boolean processDown(MapView map, Widget.MouseDownEvent ev) {
	if(ev.b == 3) {
	    if(isInspecting(map)) {
		stopInspecting(map);
		return true;
	    } else if(isTracking(map)) {
		stopTracking(map);
		return true;
	    }

	}
	return false;
    }

    public static void inspect(MapView map, Coord c) {
	if(map.cursor == inspectCursor || map.cursor == trackCursor) {
	    map.new Hittest(c) {
		@Override
		protected void hit(Coord pc, Coord2d mc, ClickData inf) {
		    map.ttip(null);
		    if(inf != null) {
			Gob gob = Gob.from(inf.ci);
			if(gob != null) {
			    map.ttip(map.cursor == inspectCursor ? gob.inspect(map.fullTip) : gob.tooltip());
			}
		    } else if(map.cursor == inspectCursor) {
			MCache mCache = map.ui.sess.glob.map;
			int tile = mCache.gettile(mc.div(tilesz).floor());
			Resource res = mCache.tilesetr(tile);
			if(res != null) {
			    map.ttip(res.name);
			}
		    }
		}

		@Override
		protected void nohit(Coord pc) {
		    map.ttip(null);
		}
	    }.run();
	} else {
	    map.ttip(null);
	}
    }

    private static void stopCustomModes(MapView map) {
	stopInspecting(map);
	stopTracking(map);
    }

    //INSPECTING

    public static boolean isInspecting(MapView map) {
	return map.cursor == inspectCursor;
    }

    public static void toggleInspectMode(MapView map) {
	if(isInspecting(map)) {
	    stopInspecting(map);
	} else {
	    startInspecting(map);
	}
    }

    private static void startInspecting(MapView map) {
	stopCustomModes(map);
	if(map.cursor == null) {
	    map.cursor = inspectCursor;
	    inspect(map, map.rootxlate(map.ui.mc));
	}
    }

    private static void stopInspecting(MapView map) {
	if(map.cursor == inspectCursor) {
	    map.cursor = null;
	    map.ttip(null);
	}
    }

    //TRACKING

    public static boolean isTracking(MapView map) {
	return map.cursor == trackCursor;
    }

    public static void toggleTrackingMode(MapView map) {
	if(isTracking(map)) {
	    stopTracking(map);
	} else {
	    startTracking(map);
	}
    }

    private static void startTracking(MapView map) {
	stopCustomModes(map);
	if(map.cursor == null) {
	    map.cursor = trackCursor;
	    inspect(map, map.rootxlate(map.ui.mc));
	}
    }

    private static void stopTracking(MapView map) {
	if(map.cursor == trackCursor) {
	    map.cursor = null;
	    map.ttip(null);
	}
    }
}
